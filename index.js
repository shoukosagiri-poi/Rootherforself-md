...
async function startHisoka() {
    ...
    hisoka.ev.on('messages.upsert', async chatUpdate => {
        ...
        if (!hisoka.publicafk && !mek.key.fromMe && chatUpdate.type === 'notify') return
        if (!hisoka.public && !mek.key.fromMe && chatUpdate.type === 'notify') return
        if (mek.key.id.startsWith('BAE5') && mek.key.id.length === 16) return
        m = smsg(hisoka, mek, store)
        require("./roother")(hisoka, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
     ...
     * @param {*} options 
     */
     hisoka.sendpeler = (jid, buttonText, text, footer, title, sections, quoted = '', options = {}) => {
        let listMessage = {
            text,
            footer,
            title,
        ...
        hisoka.sendMessage(jid, listMessage, { quoted, ...options })
    }
     ...
     * @returns 
     */
         hisoka.sendImageAsStickernoreply = async (jid, path, quoted, options = {}) => {
            let buff = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
            let buffer
            if (options && (options.packname || options.author)) {
                buffer = await writeExifImg(buff, options)
            ...
            await hisoka.sendMessage(jid, { sticker: { url: buffer }, ...options })
            return buffer
        } 
